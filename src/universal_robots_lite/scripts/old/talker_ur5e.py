...
def main():
    rospy.init_node('talker_ur5e', anonymous=True)

    # Get the UR5 IP address from parameter server
    ur5_ip = rospy.get_param('~ur5_ip', '192.168.1.104')

    # Log the IP for debugging purposes
    rospy.loginfo("Using UR5 IP: %s", ur5_ip)
    ur5_pub_sub_instance = ur5_pub_sub(ur5_ip)
    ur5_pub_sub_instance.pose_publisher()
